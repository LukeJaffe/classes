...
def get_p_cl(z1_val, z2_val):
    cl = bayes_net["cond_likelihood"][(z1_val, z2_val)].squeeze()
    return cl
...
def get_p_x_cond_z1_z2(z1_val, z2_val):
	...
	'''
        ...
        for i in range(sample.shape[0]):
            ...
            sample[i] = np.random.choice([1,0], p=[cl, 1-cl])
...
def get_conditional_expectation(data):
    ...
    def calc_means(X): 
        ...
        joint_arr = np.zeros((len(disc_z1), len(disc_z2)))
        ...
        for i, z1_val in enumerate(disc_z1):
            for j, z2_val in enumerate(disc_z2):
                ...
                # Get terms from net
                cl = cond_likelihood[(z1_val, z2_val)].squeeze()
                z1 = prior_z1[z1_val]
                z2 = prior_z2[z2_val]
                ...
                # Add z terms to sum
                z_sum += z_vals*jp
        # Calculate normal log jp
        norm_term = np.sum(joint_arr)
        # Normalize z_vals
        z_mean = z_sum/norm_term
...
def q6():
	...
	test_data = mat['test_x']
        ...
        # Calculate max log-likelihood naively (not used, was for debug)
        def calc_mll(X): 
            # Convert input image to binary mask
            mask1 = X.astype(bool)
            # Make a mask for X=0 for readability
        ...
        mll_file = "mll.npy"
        val_mll_arr = None
        if not os.path.isfile(mll_file):
            ...
            for k,v in enumerate(val_data):
                ...
                val_mll_arr[k] = mll
            np.save(mll_file, val_mll_arr)
        else:
            val_mll_arr = np.load(mll_file)
        ...
        print mll_lower_bound, mll_upper_bound
