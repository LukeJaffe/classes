...
class Factor:
    ...
    def marginalize_all_but(self, var, marginal_type="sum"):
        ...
        this implementation shouldn't contain too many surprises.  It is however a nontrivial
        amount of code (25 lines by our reckoning)
        """
        assert marginal_type=="sum" or marginal_type=="max"
        g = Factor() #modify this factor to make it the marginalized Factor, and then return it.
        ...
        if(marginal_type=="sum"):
            g = self.marginalizeAllBut(var)
        elif(marginal_type=="max"):
            g = self.maxMarginalizeAllBut(var);
        return g   
    
    def marginalizeAllBut(self, var):
